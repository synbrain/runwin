...
# Nếu không còn sử dụng kiểm tra mount /mnt, đảm bảo rằng thư mục /mnt đã tồn tại
if [ ! -d /mnt ]; then
...
# Giải nén các file .zip trong thư mục hiện tại
echo "Đang giải nén tất cả các file .zip trong thư mục hiện tại..."

for zip_file in ./*.zip; do
    unzip "$zip_file" -d .
    if [ $? -ne 0 ]; then
        echo "Lỗi khi giải nén $zip_file. Vui lòng kiểm tra lại file tải về."
        exit 1
    fi
done
